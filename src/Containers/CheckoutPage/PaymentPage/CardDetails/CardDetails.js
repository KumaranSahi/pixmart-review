...
const CardDetails=({paymentMode,setAddpayment})=>{
    ...
    const cardNumberEntered=(event)=>{
        if(!isNaN(event))
            setCardNumber(event)
        setCardNumber(number=>number.replace(/^(\d{4})(\d{4})(\d{4})(\d{4})$/,"$1 $2 $3 $4"))
    }
    ...
    const addClicked=(event)=>{
        ...
        if(checkName(nameOnCard)&&checkNumber(cardNumber)&&checkExpirationDate(expirationDate)&&checkCvv(cvv)){
            addNewPayment({
    ...
    return(
        <form className={classes["card-details"]} onSubmit={addClicked}>
            ...
            />
            {!cardNumberValid&&<p className={classes["error-text"]}>Please enter a valid card 16 digit card number</p>}
            ...
            {!(cvvValid&&expirationDateValid)&&<p className={classes["error-text"]}>Please verify your cvv and expiration date</p>}
            <button 
                type="submit"
